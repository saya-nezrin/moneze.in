...
function createIdentityVerificationSession(identity) {
  const now = Math.floor(Date.now() / 1000);
  const payload = Buffer.from(JSON.stringify({ ...identity, issuedAt: now, expiresAt: now + SESSION_EXPIRY_SECONDS })).toString("base64url");
  return `${payload}.${digest(`session:${payload}`)}`;
}

export function createVerificationSession(email) {
  return createIdentityVerificationSession({ email });
}

export function createPhoneVerificationSession(phone) {
  return createIdentityVerificationSession({ phone });
}
...
export function verifyVerificationSession(token) {
  ...
  try {
    ...
    const validIdentity = isValidEmail(session.email || "") || isValidIndianPhone(session.phone || "");
    if (!validIdentity || session.expiresAt < Math.floor(Date.now() / 1000)) return null;
    return session;
  } catch {
    return null;
  }

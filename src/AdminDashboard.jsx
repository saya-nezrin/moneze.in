...
export default function AdminDashboard() {
  ...
  return <main className="admin-page">
    ...
    <section className="admin-container">
      ...
      <div className="admin-tabs" role="tablist" aria-label="Admin dashboard sections"><button type="button" role="tab" aria-selected={activeTab === "leads"} aria-controls="admin-leads-panel" onClick={() => setActiveTab("leads")}>Consultation leads</button><button type="button" role="tab" aria-selected={activeTab === "analytics"} aria-controls="admin-analytics-panel" onClick={() => setActiveTab("analytics")}><BarChart3 size={18} /> Analytics</button></div>
      <div id="admin-analytics-panel" role="tabpanel" hidden={activeTab !== "analytics"}>
      ...
      {analytics.data && <section className="admin-ga-pages"><div><p>ENGAGEMENT</p><h2>Clicks, scrolls and website events</h2></div>{analytics.data.events?.length ? <div>{analytics.data.events.map((event) => <article key={event.name}><span>{event.name.replaceAll("_", " ")}</span><strong>{event.count.toLocaleString("en-IN")}</strong></article>)}</div> : <p className="admin-ga-empty">Engagement reports will appear as Google Analytics processes new activity.</p>}</section>}
      </div>
      <div id="admin-leads-panel" role="tabpanel" hidden={activeTab !== "leads"}>
